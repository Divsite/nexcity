<template>
    <div :class="field.width">
        <div class="mb-3">
            ...
            <template v-if="field.input_group">
                <div class="input-group">
                    <span class="input-group-text" v-if="field.left_text_input_group">
                        {{ field.left_text_input_group }}
                    </span>
                    <input type="text" v-model="value[field.id]" :id="field.id" class="form-control"
                           :placeholder="field.placeholder" :disabled="disableField()"
                           :maxlength="field.max_char_limit">
                    <span class="input-group-text rounded-end" v-if="field.right_text_input_group">
                        {{ field.right_text_input_group }}
                    </span>
                </div>
            </template>
            <template v-else>
                <input type="text" v-model="value[field.id]" :id="field.id" class="form-control"
                       :placeholder="field.placeholder" :disabled="disableField()" :maxlength="field.max_char_limit">
            </template>
