    ...
    <div :class="field.width">
        <div class="mb-3">
            <div :id="field.id" class="form-label fw-medium">
                {{ field.label }}
                <span class="text-danger ms-1" v-if="requiredField()">*</span>
            </div>
            <file-pond
                ref="pond"
            ...
            />
            <div class="invalid-feedback d-block mt-1" v-if="form.errors[field.id]">
...
export default {
    ...
    data() {
        return {
            ...
            server: {
                process: {
                    url: route('form-submission-files.temp.store', {
                        id: useFormSubmissionStore().id,
                        uuid: this.field.id
                    }),
                    method: 'POST',
                    headers: {
                    ...
                    onerror: (response) => {
                        let data = JSON.parse(response);
                        this.trans.error_during_upload = data.message;
                    }
                },
                revert: {
                    url: route('form-submission-files.temp.destroy'),
                    method: 'DELETE',
                    headers: {
                        'Accept': 'application/json',
                        'X-CSRF-TOKEN': window.Laravel.csrfToken,
