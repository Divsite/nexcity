...
namespace App\Http\Livewire\FormTypes;
...
class FormTypeTable extends DataTableComponent
{
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make(__('messages.actions'))
                ->label(
                    fn($row, Column $column) => view('form-types.columns.table-actions')->withRow($row)
                )
                ->hideIf(
                    !auth()->user()->can('read-form-types') &&
                    !auth()->user()->can('edit-form-types') &&
                    !auth()->user()->can('delete-form-types')
                )
        ];
    }
    ...
    {
        if (auth()->user()->cannot('delete-form-types')) {
        ...
        return to_route('form-types.index');
    }
