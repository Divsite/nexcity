...
class MyCompletedTaskTable extends DataTableComponent
{
    ...
    public function builder(): Builder
    {
        return FormSubmission::with(['form', 'currentStatus'])
            ...
            ->whereHas('completedTasks', function (Builder $query) {
                $query->where('user_id', '=', auth()->user()->id);
            });
    }
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make(__('messages.actions'))
                ->label(
                    fn($row, Column $column) => view('tasks.columns.table-actions')->withRow($row)
                ),
        ];
    }
}
