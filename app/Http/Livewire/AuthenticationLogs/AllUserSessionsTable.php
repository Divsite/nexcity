...
namespace App\Http\Livewire\AuthenticationLogs;
...
class AllUserSessionsTable extends DataTableComponent
{
    ...
    public function configure(): void
    {
        $this->setPrimaryKey('id')
            ->setAdditionalSelects(['authentication_log.id', 'authentication_log.authenticatable_type', 'authentication_log.authenticatable_id'])
    ...
    {
        return [
            Column::make(__('messages.name'), 'authenticatable_id')
                ->sortable(function(Builder $builder, string $direction) {
                    ...
                    })->orderBy('users.name', $direction)->select(['users.*', 'authentication_log.*']);
                })
                ->format(
                    fn($value, $row, Column $column) => $row->authenticatable->name,
                ),
        ...
        return [
            TextFilter::make(__('messages.name'), 'name')
                ...
                ->filter(function(Builder $builder, string $value) {
                    $builder->whereHas('authenticatable', function (Builder $query) use ($value) {
